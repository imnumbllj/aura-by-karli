    ...
    <div style={{ position: 'relative' }}>
      ...
      {open && (matches.length > 0 || showAdd) && (
        <div style={{
          ...
          borderRadius: 9, boxShadow: '0 12px 40px rgba(0,0,0,0.6)', overflow: 'hidden',
        }}>
          {matches.map(p => (
...
function CompraForm({ onClose, productos, onRegistrar, onEditar, orderEdit }) {
  ...
  const [items, setItems] = useState(() =>
    orderEdit
      ? orderEdit.items.map(it => ({
          producto: it.Producto, categoria: it.Categoria || CATEGORIAS[0],
          unidad: it.Unidad || 'Unidad', cantidad: String(it.Cantidad),
          costoTotal: String(it['Costo T']), esNuevo: false,
        }))
      : [EMPTY_ITEM()]
  );
  ...
  const updateProducto = (i, nombre, productData) => {
    setItems(p => p.map((it, x) => {
      if (x !== i) return it;
      if (productData) return { ...it, producto: nombre, categoria: productData.categoria || it.categoria, unidad: productData.unidad || it.unidad, esNuevo: false };
      return { ...it, producto: nombre, esNuevo: false };
    }));
  };

  const markNuevo = (i, nombre) => setItems(p => p.map((it, x) => x === i ? { ...it, producto: nombre, esNuevo: true } : it));
  const update    = (i, field, value) => setItems(p => p.map((it, x) => x === i ? { ...it, [field]: value } : it));
  ...
  const handleSubmit = (e) => {
    ...
    const enriched = valid.map(it => ({
      producto: it.producto.trim(), categoria: it.categoria, unidad: it.unidad,
      cantidad: Number(it.cantidad), costoTotal: Number(it.costoTotal), esNuevo: it.esNuevo,
    }));
    ...
    <Modal onClose={onClose} title={isEdit ? `Editando ${orderEdit.id}` : 'Nueva Compra'} subtitle={isEdit ? `Orden del ${orderEdit.fecha}` : 'Registra los productos adquiridos'} icon={ShoppingCart} maxWidth={700}>
      <form onSubmit={handleSubmit}>
        ...
        <button type="button" onClick={addItem}
          style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 500, color: t.accent, background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: 'inherit', marginBottom: 4 }}>
        ...
        <ModalFooter>
          ...
          <Btn type="submit" variant="primary">
            {isEdit ? 'Guardar cambios' : nuevosCount > 0 ? `Registrar + ${nuevosCount} nuevo${nuevosCount > 1 ? 's' : ''}` : 'Registrar compra'}
          </Btn>
        </ModalFooter>
      </form>
...
export default function Compras() {
  ...
  const totalOrdenes = useMemo(() => [...new Set(compras.map(c => c.ID))].length, [compras]);
  ...
  const buildNuevos = (items) => {
    ...
    const batch = nuevos.map(it => {
      maxId++;
      return { id: `P${String(maxId).padStart(3, '0')}`, nombre: it.producto.trim(), variante: '', nombreCompleto: it.producto.trim(), categoria: it.categoria, tipo: 'Directo', unidad: it.unidad, activo: true };
    });
    agregarBatch(batch);
  };

  const handleRegistrar = (items, fecha) => { buildNuevos(items); registrar(items, fecha); };
  const handleEditar    = (orderId, items, fecha) => { buildNuevos(items); editar(orderId, items, fecha); };
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="show" style={{ maxWidth: 860 }}>
      ...
      {/* Search */}
      <motion.div variants={fadeUp} style={{ position: 'relative', marginBottom: 14 }}>
            ...
            {confirmDel === g.id ? (
              <div style={{ padding: '13px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(239,68,68,0.07)', flexWrap: 'wrap', gap: 10 }}>
                <div>
                  ...
                  <p style={{ fontSize: 11, color: t.text3, marginTop: 2 }}>Se revertirá el inventario de {g.items.length} producto{g.items.length !== 1 ? 's' : ''}</p>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <Btn size="sm" variant="ghost"  onClick={() => setConfirmDel(null)}>Cancelar</Btn>
                  <Btn size="sm" variant="danger" onClick={() => { eliminar(g.id); setConfirmDel(null); }}>Sí, eliminar</Btn>
                </div>
              </div>
            ) : (
                  ...
                  <tbody>
                    {g.items.map((item, i) => (
                      <tr key={i} style={{ borderTop: i > 0 ? `1px solid ${t.border}` : 'none' }}
                        ...
                        <td style={{ padding: '10px 14px' }}><Badge color={CAT_COLOR[item.Categoria] || 'gray'}>{item.Categoria}</Badge></td>
